  ...
  return (
    <div className="rounded-2xl border border-border/60 bg-surface p-5">
      <div className="flex items-center justify-between">
        <div className="text-xs uppercase tracking-wide text-muted">Flare log</div>
        {todayFlare ? (
          <button onClick={onUnmark} className="text-xs text-muted underline underline-offset-4 hover:text-text">
            Unmark today
          </button>
        ) : (
          <button
            onClick={onMark}
            className="rounded-lg bg-high px-3 py-1.5 text-sm font-medium text-white hover:bg-high"
          >
      ...
      {todayFlare && (
        <div className="mt-4 space-y-3">
          <div className="text-sm text-high-ink">Today is marked as a flare.</div>
          <div>
            <div className="text-sm text-text">What’s helping?</div>
      ...
      {works.length > 0 && (
        <div className="mt-5">
          <div className="text-sm font-medium text-text">What works for you</div>
          <p className="text-xs text-muted">Across {flares.length} logged flares.</p>
          <ul className="mt-2 space-y-1.5">
            {works.slice(0, 5).map((w) => (
              <li key={w.remedy} className="flex items-center gap-2 text-sm text-text">
      ...
      {flares.length > 0 && (
        <div className="mt-5">
          <div className="text-sm font-medium text-text">Past flares</div>
          <ul className="mt-2 space-y-2">
            {flares.slice(0, 8).map((f) => (
              <li key={f.dateKey} className="rounded-lg border border-border/50 px-3 py-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-text">{formatDate(f.dateKey)}</span>
                  {f.pain != null && <span className="text-xs text-muted">pain {f.pain}/10</span>}
                </div>
                {f.flare.helped?.length > 0 && (
                  <div className="mt-1 text-xs text-good-ink">helped: {f.flare.helped.join(', ')}</div>
                )}
                {f.flare.note && <div className="mt-1 text-xs text-muted">“{f.flare.note}”</div>}
              </li>
            ))}
          </ul>
        </div>
